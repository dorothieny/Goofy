    ...
    return (
      <React.Fragment>
        ...
        <button className="add_smth"
         onClick={() => location.href = `http://127.0.0.1:3000/types/new`} >
         +
          </button>
          </div>  
          ...
          <tbody>
            {this.props.types.map(type => (
             <tr key={type.id} 
              ...
              onClick={() => location.href = `http://127.0.0.1:3000/types/${type.id}`}>{type.name}</td>
              ...
              <button style={{ marginRight: '10px' }}
                onClick={() => location.href = `http://127.0.0.1:3000/types/${type.id}/edit`}>Edit</button>
                <a 
